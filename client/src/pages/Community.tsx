        ...
        <div className='py-10 min-h-[80vh]'>
          ...
          <div className="flex flex-wrap gap-3.5">
            {projects.map((project)=>(
              ...
              to={`/view/${project.id}`} 
              target='_blank'
              key={project.id} className='w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden group hover:border-indigo-800/80 transition-all duration-300'>
                {/* Desktop-like mini Preview */}
                <div className="relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800">
                  {project.current_code ?(
                    <iframe
                    srcDoc={project.current_code}
                    className='absolute top-0 left-0 w-300 h-200 origin-top-left text-gray-500'
                  ...
                  <div className="flex justify-between items-center mt-6">
                    <span className='text-xs text-gray-500'>
                      ...
                      <div className="flex gap-3 text-white text-sm">
                        <button className='px-3 p-1.5 bg-white/10 hover:bg-white/15 rounded-md transition-colors flex items-center gap-2'>
